...
class Wordify(object):
    ...
    def __call__(self, data, *args, **kwargs):
        ...
        # create features
        ngram_vectorizer = TfidfVectorizer(analyzer='word', ngram_range=(1, 3), min_df=0.001, 
                                        max_df=0.75, stop_words=self.stopwords, sublinear_tf=True)
    ...
    def _wordify(self, X, y, feature_names):
        ...
        for iteration in range(self.num_iters):
            ...
            clf = LogisticRegression(penalty='l1', C=PENALTIES[np.random.randint(len(PENALTIES))],
                                    solver='liblinear', multi_class='ovr', max_iter=500)
            selection = resample(np.arange(n_instances), replace=True, 
                                stratify=y, n_samples=sample_size)    
            try:
        ...
        # prepare DataFrame
        pos = [(feature_names[i], pos_scores[c, i], self.le.classes_[c]) for c, i in zip(*pos_positions.nonzero())]
        neg = [(feature_names[i], neg_scores[c, i], self.le.classes_[c]) for c, i in zip(*neg_positions.nonzero())]
        posdf = pd.DataFrame(pos, columns='term score label'.split()).sort_values(['label', 'score'], ascending=False)
        negdf = pd.DataFrame(neg, columns='term score label'.split()).sort_values(['label', 'score'], ascending=False)
