    ...
    if form.validate_on_submit():
       # collect the client inputs
        ...
        if ('label' in df.columns) and ('text' in df.columns):
            ...
            session['min_labels'] = [label for label, too_little in (df['label'].value_counts() < MIN_LABELS).items() if too_little]

            error_message = []

            if session.get('nrow') <= MIN_ROWS:
                error_message.append('Your file has less than {} texts.'.format(MIN_ROWS))
            if session.get('min_labels'):
                error_message.append('Some labels ({}) occur fewer than {} times.'\
                    .format(', '.join(session.get('min_labels')), MIN_LABELS))
            if error_message:
                error_message.insert(0, 'WARNING:')
                error_message.append('We will still process your data, \
            ...
            message = 'The column names are wrong. Please use "label" and "text" and try again.'
            flash(message)
